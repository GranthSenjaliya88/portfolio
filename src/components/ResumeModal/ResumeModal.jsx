  ...
  useEffect(() => {
    ...
    return () => {
      ...
      requestAnimationFrame(() => {
        // A mobile-menu link is removed when the menu closes.
        const trigger = returnFocusRef.current;
        if (trigger?.isConnected && trigger.getClientRects().length) trigger.focus({ preventScroll: true });
        else document.querySelector('[aria-controls="mobile-menu"]')?.focus({ preventScroll: true });
      });
    };
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div className="resume-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
          <motion.div ref={dialogRef} data-lenis-prevent className="resume-dialog" role="dialog" aria-modal="true" aria-labelledby="resume-title" initial={{ opacity: 0, y: 40, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 30, scale: .98 }} transition={{ ease: [.16, 1, .3, 1] }}>
            ...
            <div className="resume-dialog-actions">
              <a href="./Granth_Senjaliya_Resume.pdf" download><Download size={17} /> Download PDF</a>
              <a href="./Granth_Senjaliya_Resume.pdf" target="_blank" rel="noreferrer"><ExternalLink size={17} /> Open PDF</a>
            </div>
          </motion.div>
        </motion.div>
      )}
